...
WRITE PROC near ; in: DX = offset string
    push AX
    mov ah, 09h
    int 21h
    pop AX
...
WRITE_LINE PROC near ; in: DX = offset string
    call WRITE
    mov DX, offset ENDL
    call WRITE
    ret
...
DEC_WORD_PRINT PROC NEAR ; if: AX = short
        push ax
        push cx
        push dx
        push bx
...
HEX_BYTE_PRINT PROC NEAR ; in: AL = byte
        push ax
        push bx
        push dx
...
HEX_WORD_PRINT PROC	NEAR ; in: AX = short
        push ax
        push ax
        mov al, ah
        call HEX_BYTE_PRINT
